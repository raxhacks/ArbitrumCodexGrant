...
const NODE_INTERFACE_ABI = [
    ...
    },
    {
        ...
        type: "function",
        stateMutability: "view",
        inputs: [{ name: "l1BlockNum", type: "uint64" }],
        outputs: [
...
async function getL2ToL1BlockMapping() {
    ...
    const arbSys = new l2Web3.eth.Contract(ARB_SYS_ABI, ARB_SYS_ADDRESS);
    const nodeInterface = new l2Web3.eth.Contract(NODE_INTERFACE_ABI, NODE_INTERFACE_ADDRESS);

    // Resolve L2 block number
    let l2BlockNum;
    if (L2_BLOCK_NUMBER === "latest") {
        l2BlockNum = await arbSys.methods.arbBlockNumber().call();
    ...
    const l2Block = await l2Web3.eth.getBlock(Number(l2BlockNum));
    if (!l2Block) {
    ...
    console.log("L2 block timestamp:", new Date(Number(l2Block.timestamp) * 1000).toISOString());
    ...
    const l1Block = await l1Web3.eth.getBlock(Number(l1BlockNum));
    if (l1Block) {
        console.log("L1 block hash:", l1Block.hash);
        console.log("L1 block timestamp:", new Date(Number(l1Block.timestamp) * 1000).toISOString());
        console.log("L1 transactions:", l1Block.transactions.length);

        // Time difference
        const timeDiff = Number(l2Block.timestamp) - Number(l1Block.timestamp);
    ...
    const mappingResults = await Promise.allSettled(
        blockNums.map((i) => nodeInterface.methods.blockL1Num(i).call().then((l1) => ({ i, l1 })))
    );
